...
import gauth from "@googleapis/oauth2";
...
const client_secret = (process.env.CLIENT_SECRET || "").replace(/\\n/g, "\n");
...
function checkItemSkus(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (itemId == null || itemId === "00000000-0000-0000-0000-000000000000")
      return false;
    ...
    if (
      ...
      itemSku.toLowerCase().includes("roller") ||
      itemSku.toLowerCase().includes("rack") ||
      itemSku.toLowerCase().includes("ball")
    )
      return false;
  }
  return true;
...
export async function handler(event) {
  ...
  try {
    ...
    await client2.refreshAccessToken();
        ...
        const sendSnsMsg = handleRetries(async () => {
          await sns
            .publish({
              ...
              Subject: "SPORTS 1 CRITICAL PRINT ERROR",
    ...
    console.log("SPORTS 1 AUTOMATION FAILED!");
    const errorMsg =
      err instanceof Error
        ? `ERROR: ${err.message}\nSTACK: ${err.stack}`
        : `ERROR: ${JSON.stringify(err, null, 2)}`;
